...
class Uris:
    ...
    def _uris_patcher(self):
        patches = self._create_patches()
        chunk = 0

        for patch in patches:
            loop = asyncio.get_event_loop()
            future = asyncio.ensure_future(self._main(patch))
            responses = loop.run_until_complete(future)
            self._save_uris(responses, chunk)
            chunk += 1
    ...
    def _create_patches(self):
        ...
        return [queries[x:x + 800] for x in range(0, len(queries), 800)]
